...
\EnglishTitle{Typesetting Bibliographies Compliant with
  the International Standard ISO 690 in \LaTeX{}}
\author{Dávid Lupták}
\podpis{Dávid Lupták, \texttt{422640@mail.muni.cz}\\
...
štýlu\footnote{\label{footnote:ambiguous:style:names} Často nazývaný
aj citačný štýl, v~tomto kontexte by bolo možné hovoriť všeobecne
o~formátovacom štýle.} (rieši problém~\ref{item:2}) vygeneruje
\LaTeX{}ové prostredie \texttt{thebibliography} s~bibliografiou na
vysadenie. Typickým zástupcom tohoto prístupu je \BibTeX{}, ktorého
nespornou výhodou je práve spomínané oddelenie formy od obsahu.
...
Pre kompletnosť tejto sekcie je žiadúce uviesť aj samotný formát
...
\printbibliography[title={Zoznam bibliografických citácií}]
...
\begin{summary}
Preparation of bibliographic references and citations complying with
the ISO 690 international standard is required by many institutes
not limited to the Czech academic sphere. However, typesetting the
bibliography conforming to the respective standard is not supported
in the \LaTeX{} document preparation system yet. Based on the initial
implementation, the package \textsf{biblatex-iso690} has been revised
and corrected to fully meet the requirements of the international
standard and thus greatly simplifies the way of typesetting the
bibliography of all kinds of information resources.
\end{summary}
